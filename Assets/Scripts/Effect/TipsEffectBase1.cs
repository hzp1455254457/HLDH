...
public class TipsEffectBase1 : MonoBehaviour
{
   [Header("所有需要显示img组件")]
    public Image[] Img;
   [Header("所有需要显示text组件")]
  ...
  [Header("左右间隔宽度")]
    ...
    public virtual void Show(Vector3 targetTf, string[] tipsValue, Sprite[] sprite, UnityAction unityAction, Color[] color, float scale = 1f)
    {
    ...
    protected virtual void Animation(Vector3 targetTf)
    {
        foreach (var item in graphics)
        {
            item.DOFade(1, 1f).SetUpdate(true);
        }
        transform.DOMoveY(targetTf.y, 0.6f).SetEase(Ease.OutQuint).SetUpdate(true).onComplete += Animation;
    }
}
